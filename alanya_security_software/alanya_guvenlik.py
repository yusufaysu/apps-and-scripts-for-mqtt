...
def read_licenses_as_3d_array(filename):
    """
    CSV dosyasından daire, lisans ve TİP bilgilerini döner.
    """
    daires = []
    licenses = []
    types = []
...
class App(ctk.CTk):
    ...
    def on_button_click(self, button_type):
        if button_type == 1:  # Su Aç
            ...
            topic = f"/{self.get_licanse_from_daire_no(self.option_menu.get())}/devListener"
            if self.get_type_from_daire_no(self.option_menu.get()) == "1":
                payload = """{"com":"event","id":6,"durum":{"stat":false}}"""
            else:
                payload = """{"com":"event","id":15,"durum":{"stat":false}}"""
            ...
            topic = f"/{self.get_licanse_from_daire_no(self.option_menu.get())}/devListener"
            if self.get_type_from_daire_no(self.option_menu.get()) == "1":
                payload = """{"com":"event","id":6,"durum":{"stat":true}}"""
            else:
                payload = """{"com":"event","id":15,"durum":{"stat":true}}"""
            result = self.client.client.publish(topic, payload)
            if result[0] != 0:
                self.show_log(f"Server Publish failed with code: {result[0]}", "red")
            ...
            #anakutu ise
            if self.get_type_from_daire_no(self.option_menu.get()) == "1":
                payload = """{"com":"event","id":7,"durum":{"stat":false}}"""
            #ekkutu ise
            else:
                payload = """{"com":"event","id":16,"durum":{"stat":false}}"""
            result = self.client.client.publish(topic, payload)
            if result[0] != 0:
                self.show_log(f"Server Publish failed with code: {result[0]}", "red")
            ...
            #anakutu ise
            if self.get_type_from_daire_no(self.option_menu.get()) == "1":
                payload = """{"com":"event","id":7,"durum":{"stat":true}}"""
            #ekkutu ise
            else:
                payload = """{"com":"event","id":16,"durum":{"stat":true}}"""
            result = self.client.client.publish(topic, payload)
            if result[0] != 0:
                self.show_log(f"Server Publish failed with code: {result[0]}", "red")
